...
Ext.ux.tree.TreeGrid = Ext.extend(Ext.tree.TreePanel, {
    ...
    initComponent:function ()
    {
        ...
        if (!this.internalTpl)
        {
            this.internalTpl = new Ext.XTemplate(
                    ...
                    '<tpl for="columns">',
                    '<td class="x-grid3-hd x-grid3-cell x-treegrid-hd" style="text-align: {align};" id="', this.id,
                    '-xlhd-{#}">',
        ...
        if (!this.rowsTpl)
        {
            this.rowsTpl = new Ext.XTemplate(
                    '<tr class="x-grid3-hd-row">',
                    '<tpl for="columns">',
                    '<td class="x-grid3-hd x-grid3-cell x-treegrid-hd" style="text-align: {align};" ',
    ...
    onRender:function ()
    {
        ...
        if (this.rows)
        {
            var theadCt = Ext.get(this.outerCt.query("TABLE THEAD TR")[0]);
            for (var i = 0; i < this.rows.length; i++)
            {
                this.rowsTpl.insertBefore(theadCt, {columns:this.rows[i]});
            }
        }
