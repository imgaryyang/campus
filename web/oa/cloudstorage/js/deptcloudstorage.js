...
$j(function () {
    ...
    renameEle.bind("click", function () {
        ...
        if ($j("#changeStyle").attr("styleTag") == "0") {
            ...
            a.css("display", "none");
            selectedFile.parent().parent().append("<div  style='z-index: 2'   class='srk'>" +
    ...
    newfolder.bind("click", function () {
        if ($j("#newf").length > 0) {
            ...
            if ($j("#changeStyle").attr("styleTag") == "0") {
                ...
                $j("#table2").prepend("<li  > " +
                    "<h2 ><a href=''#'><img src='../image/dwjj_03.png' width='56' height='46' /></a></h2> " +
    ...
    uploadFileBut.bind("click", function () {
        System.showModal("/oa/cloudstorage/deptcloud/batch");
    })
    ...
    downFileBut.bind("click", function () {
        ...
        if ((folderIdsStr == null || folderIdsStr == "") && (fileIds == null || fileIds == "")) {
            ...
            window.location.href = "/oa/cloudstorage/deptcloud/zip?folderIds=" + folderIdsStr + "&fileIds=" + fileIds;
        }
    })
...
function apeendCopyView(type) {
    ...
    ajax.handleObject = function (result) {
        ...
        childButA.bind("click", function () {
            ...
            $j(this).css("background", "#dfe5ea url(../image/sj_03.png) no-repeat 14px center");
    ...
    ajax.call("/oa/cloudstorage/deptcloud/createAllUserFolder?folderIds=" + folderIdsStr + "&fileIds=" + fileIds);
}
...
function deleteFile() {
    ...
    ajax.call("/oa/cloudstorage/deptcloud/deptcloudcrud/deleteFolderAndFiles?folderIds=" + folderIdsStr + "&fileIds=" + fileIds);
}
...
function spanToggle(obj) {
    ...
    if ($j(obj).attr("state") == "0") {
        ...
        ajax.call("/oa/cloudstorage/deptcloud/createFolderTreeChildren?parentId=" + parenId);
    }
}
/**
 * 关闭复制界面
...
function copyFile(seledFolderId) {
    ...
    ajax.call("/oa/cloudstorage/deptcloud/copyToTargetFolder?folderIds=" + folderIdsStr + "&fileIds=" + fileIds + "&seledFolderId=" + seledFolderId);
}
...
function copyNewFolder() {
    if ($j("#newf").length > 0) {
        ...
        root.append("<ul><li style='line-height: 36px;padding-left: 40px'><a href='#'>" +
            "<img src='../image/jjh_03.png' width='26' height='21'/>" +
            "<input style='width: 152px;height: 22px;border: none;box-shadow: 0 0 6px #2db4e0' id='newf' value='新建文件夹'/>" +
            "<img onclick='createCopyFolder()' style='margin:10px 0 0 5px' class='copyimg' src='../image/dc_03.png'><img onclick='removeCopythis(this)' style='margin:10px 0 0 5px;'  class='copyimg' src='../image/dc_05.png'></a></li> " +
            "</ul>");
        $j("#newf").focus();
        $j("#newf:text").select();
    }
        ...
        callback: function (result) {
            if (result) {
                ...
                seled.parent().append("<ul>" +
                    "<li class='childli'>" +
                    "<a style='background:#dfe5ea url(../image/sj_03.png) no-repeat 14px center' class='seled' code=" + result.folderId + " href='#'>" +
                    "<img src='../image/jjh_03.png' width='26' height='21'>" + result.folderName + "</a>" +
                    "</li> </ul>")
...
function showShareView() {
    ...
    $j("#createUrlBut").bind("click", function (result) {
        ...
        if (foerverRadio == "false" && validay == "") {
            ...
            ajax.handleObject = function (result) {
                if (result) {
                    var url = window.location.host + "/oa/cloudstorage/share/" + result.shareId;
                    var str = "";
                    if (result.password) {
...
function filesSelect(obj) {
    ...
    if ($j(obj).attr("checked")) {
        ...
        style2Check.css("background", "url(../image/xmxz_03.png)");
...
function selectFiles(obj) {
    ...
    if ($j(obj).attr("state") == "0") {
        ...
        $j(obj).css("background","url(../image/xmxz_03.png)");
...
function allcheckSelect() {
    ...
    if ($j("#allcheck").is(':checked')) {
        ...
        a2.css("background", "url(../image/xmxz_03.png)");
...
function moveFile(seledFolderId) {
    ...
    ajax.call("/oa/cloudstorage/deptcloud/moveToTargetFolder?folderIds=" + folderIdsStr + "&fileIds=" + fileIds + "&seledFolderId=" + seledFolderId);
}
    ...
    if ($j("#newf").length > 0) {
        ...
        if (seledParent.attr("id") == "root") {
            seledParent.append("<ul><li style='line-height: 36px;padding-left: 40px'><a href='#'>" +
                "<img src='../image/jjh_03.png' width='26' height='21'/>" +
                "<input style='width: 152px;height: 22px;border: none;box-shadow: 0 0 6px #2db4e0' id='newf' value='新建文件夹'/>" +
                "<img onclick='createCopyFolder(" + code + ")' style='margin:10px 0 0 5px' class='copyimg' src='../image/dc_03.png'><img onclick='removeCopythis(this," + code + ",null)' style='margin:10px 0 0 5px;'  class='copyimg' src='../image/dc_05.png'></a></li> " +
                "</ul>");
            $j("#newf").focus();
            $j("#newf:text").select();
        } else {
            ...
            parentUl.append(
                ...
                "<img src='../image/jjh_03.png' width='26' height='21'/>" +
                "<input style='width: 152px;height: 22px;border: none;box-shadow: 0 0 6px #2db4e0' id='newf' value='新建文件夹'/>" +
                "<img onclick='createCopyFolder(" + code + ")' style='margin:10px 0 0 5px' class='copyimg' src='../image/dc_03.png'><img onclick=removeCopythis(this," + code + ",'" + name + "') style='margin:10px 0 0 5px;'  class='copyimg' src='../image/dc_05.png'></a></li> "
            )

            parentUl.find("span[code=" + code + "]").css("background", "#dfe5ea url(../image/sj_03.png) no-repeat 14px center");
...
function getFolderTreeChildren(type) {
    ...
    ajax.call("/oa/cloudstorage/deptcloud/createFolderTreeChildren?parentId=0&folderIds=" + folderIdsStr + "&fileIds=" + fileIds);
}